...
export const colKeys = ["A+", "A", "B", "C", "D", "E", "F", "G", "H"] as const;
const sktMap = new Map(colKeys.map((key, index) => [key, new Map(skt.map((val, row) => [row + 1, val[index]]))]));

export type TSpalte = typeof colKeys[number];
...
export function activate(level: number, col: TSpalte) {
    level = validateLevel(level);
    return sktMap.get(col)!.get(level)!;
}

export function leverage(from: number, to: number, level: number, col: TSpalte) {
    level = validateLevel(level);
    if (from > to) {
